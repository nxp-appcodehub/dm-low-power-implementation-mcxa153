...
void WAKETIMER_EnableInterrupts(WAKETIMER_Type *base, uint32_t mask)
{
    ...
    if (0 != (mask & kWAKETIMER_WakeInterruptEnable))
    {
        base->WAKE_TIMER_CTRL |= WAKETIMER_WAKE_TIMER_CTRL_INTR_EN_MASK;
    }
}
...
{
    if (0 != (mask & (uint32_t)kWAKETIMER_WakeFlag))
    {
...
void WAKETIMER_GenericIRQHandler(WAKETIMER_Type *base, waketimer_callback_t callback)
{
...
void WAKETIMER0_DriverIRQHandler()
{
